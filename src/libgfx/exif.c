/* Hacked and slashed by Fredrik Rambris. Maybe this will work. The PHP Group
 * have done a great job cleaning up the original code. */
...
//typedef _ImageInfoType ImageInfoType;

#define E_ERROR 0

void php_error( int type, char *msg )
{
//	g_warning( msg );
}
...
*/
static void process_COM ( struct ImageInfoType *ImageInfo, uchar *Data, int length)
{
    int ch;
    char Comment[2050];
    int nch;
    ...
    if (length > 2000) length = 2000; /* Truncate if it won't fit in our structure. */
...
static void process_SOFn ( struct ImageInfoType *ImageInfo, uchar *Data, int marker)
{
...
static void ProcessExifDir( struct ImageInfoType *ImageInfo, char *DirStart, char *OffsetBase, unsigned ExifLength, char *LastExifRefd)
{
    int de;
    int a;
    int NumDirEntries;
...
*/
static void process_EXIF ( struct ImageInfoType *ImageInfo, char *CharBuf, unsigned int length, char *LastExifRefd)
{
...
static int scan_JPEG_header ( struct ImageInfoType *ImageInfo, FILE *infile, Section_t *Sections, int *SectionsRead, int ReadAll, char *LastExifRefd)
{
    int a;
    int HaveCom = FALSE;
...
/* 
   Read image data.
*/
int ReadJpegFile( struct ImageInfoType *ImageInfo, Section_t *Sections, 
				  ...
				  int ReadAll, char *LastExifRefd)
{
    ...
    if (infile == NULL) {
        php_error(E_ERROR, "Unable to open file");
        return FALSE;
    }
/*    CurrentFile = FileName; */
    ...
    if (!ret) {
        php_error(E_ERROR,"Invalid Jpeg file\n");
    }
...
int read_jpeg_exif( struct ImageInfoType *ImageInfo, char *FileName, int ReadAll)
{
